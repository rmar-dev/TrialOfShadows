...
using _Scripts.Character.Interface.Collisions.Impl;
using _Scripts.Character.Interface.DrawGizmos.API;
using _Scripts.Character.Interface.DrawGizmos.Impl;
using _Scripts.Character.Interface.Player.API;
using _Scripts.Character.Model;
using UnityEngine;

namespace _Scripts.Character.MonoBehavior.Player {
    [RequireComponent(typeof(Rigidbody2D), typeof(Collider2D))]
    public class PlayerController : MonoBehaviour, IPlayerController, ICollisionController {
        [SerializeField] private ScriptableStats _stats;
        [SerializeField] private Vector2 _gatherInputOfJump;
        ...
        private readonly IDrawGizmosHandler _drawGizmosHandler = new DrawGizmosHandlerImpl();
        private ICollisionsHandler _collisionsHandler;
        #endregion
        ...
        public CapsuleCollider2D CapsuleCollider2D => _col;
        public ScriptableStats ScriptableStats => PlayerStats;
        public bool CachedTriggerSetting => _cachedTriggerSetting;
        public Bounds StandingColliderBounds => _standingColliderBounds;
        ...
        protected virtual void TakeAwayControl(bool resetVelocity = true) {
            if (resetVelocity) {
        ...
        protected virtual void Awake() {
            ...
            _collisionsHandler = new CollisionsHandlerImpl(this);
            SetCrouching(false);
        }
        ...
        protected virtual void FixedUpdate() {
            ...
            _collisionsHandler.CheckCollisions();
            HandleCollisions();
            HandleWalls();
            HandleLedges();
            HandleLadders();
        ...
        protected virtual void HandleCollisions() {
            // Hit a Ceiling
            _speed.y = _collisionsHandler.HandleCeilingHitCount(_speed);
            // Landed on the Ground
            _collisionsHandler.DidLandedOnGround(HandleLandedOnGround);
            // Left the Ground
            _collisionsHandler.DidLeftTheGround(_fixedFrame, HandleDidLeftTheGround);
        }
        ...
        private void HandleDidLeftTheGround()
        {
        ...
        protected virtual void HandleWalls() {
            ...
            if (_collisionsHandler.WallHitCount > 0)
            {
                var position = transform.position;
                if (Physics2D.Raycast(position, Vector2.left, PlayerStats.WallDetectorSize.x))
                {
                    _wallDir = -1;

                }else if (Physics2D.Raycast(position, Vector2.right, PlayerStats.WallDetectorSize.x))
                {
                    _wallDir = 1;
                }
            }
            ...
            bool ShouldStickToWall() {
                if (_wallDir == 0 || _collisionsHandler.Grounded) return false;
        ...
        private bool CanEnterLadder => _collisionsHandler.LadderHitCount > 0 && _fixedFrame > _frameLeftLadder + _stats.LadderCooldownFrames;
        ...
        protected virtual void HandleLadders() {
            ...
            else if (_onLadder && _collisionsHandler.LadderHitCount == 0) ToggleClimbingLadders(false);

            // Snap to center of ladder
            if (_onLadder && _frameInput.Move.x == 0 && _stats.SnapToLadders && _hasControl) {
                var pos = _rb.position;
                _rb.position = Vector2.SmoothDamp(pos, new Vector2(_collisionsHandler.LadderHits[0].transform.position.x, pos.y), ref _ladderSnapVel, _stats.LadderSnapSpeed);
            }
        }
        ...
        private bool CanUseCoyote => _coyoteUsable && !_collisionsHandler.Grounded && _fixedFrame < _collisionsHandler.FrameLeftGrounded + _stats.CoyoteFrames;
        ...
        protected virtual void HandleJump() {
            if (_jumpToConsume || HasBufferedJump) {
                if (_collisionsHandler.Grounded || _onLadder || CanUseCoyote) NormalJump();
            ...
            if (!_endedJumpEarly && !_collisionsHandler.Grounded && !_frameInput.JumpHeld && _rb.velocity.y > 0) _endedJumpEarly = true; // Early end detection
        }
        ...
        protected virtual void HandleDash() {
            ...
            if (_dashing) {
                ...
                if (_fixedFrame > _startedDashing + _stats.DashDurationFrames) {
                    ...
                    if (_collisionsHandler.Grounded) _canDash = true;
                }
            }
        ...
        protected virtual void HandleHorizontal() {
            ...
            if (_frameInput.Move.x != 0) {
                if (_crouching && _collisionsHandler.Grounded) {
                    ...
                    if (_collisionsHandler.WallHitCount > 0 && Mathf.Approximately(_rb.velocity.x, 0) && Mathf.Sign(_frameInput.Move.x) == Mathf.Sign(_speed.x))
                        _speed.x = 0;
                ...
                _speed.x = Mathf.MoveTowards(_speed.x, 0, (_collisionsHandler.Grounded ? _stats.GroundDeceleration : _stats.AirDeceleration) * Time.fixedDeltaTime);
        }
        ...
        protected virtual void HandleVertical() {
            ...
            // Grounded & Slopes
            if (_collisionsHandler.Grounded && _speed.y <= 0f) {
        ...
        {
            var position = transform.position;
            var colliderOrigin = position + _standingColliderBounds.center;
            var bounds = new Bounds(colliderOrigin, PlayerStats.WallDetectorSize);
            _drawGizmosHandler.DrawGizmos(PlayerStats, bounds, _wallDir, position);
        }
    #endif
    }
}